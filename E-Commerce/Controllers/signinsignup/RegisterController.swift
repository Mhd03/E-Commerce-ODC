...
class RegisterController: UIViewController {
    ...
    @objc func didTapSignUp() {
        let registerUserRequest = RegiserUserRequest(
         ...
         password: self.emailField.text ?? ""
        )
    /*
        if !Vaildator.isVaildUsername(for: registerUserRequest.username){
            AlertManager.showInvalidUsernameAlert(on: self)
        }
        if !Vaildator.isVaildEmail(for: registerUserRequest.email){
                AlertManager.showInvalidEmailAlert(on: self)
        }
     */
        /*
        if !Vaildator.ispasswordVaild(for: registerUserRequest.password){
...
extension RegisterController: UITextViewDelegate {
    ...
    private func showWebViewerController(with urlString: String) {
        let vc7 = WebViewerController(with: urlString)
        let nav = UINavigationController(rootViewController: vc7)
