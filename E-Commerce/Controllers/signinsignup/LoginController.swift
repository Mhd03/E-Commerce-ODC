...
class LoginController: UIViewController {
    ...
    // MARK: - UI Components
    private let headerView = AuthHeaderView(title: "Welcome!", subTitle: "please login or sign up to continue our app")
    ...
    @objc private func didTapSignIn() {
        let vc6 = UIStoryboard(name:"Main", bundle:nil).instantiateViewController(withIdentifier: "Home") as! Home
    ...
    let loginRequest = LoginUserRequest(
       ...
       /*
        if !Vaildator.isVaildEmail(for: loginRequest.email){
                AlertManager.showInvalidEmailAlert(on: self)
            return
        }
        */
        /*
        if !Vaildator.ispasswordVaild(for: loginRequest.password){
     ...
     /*
        AuthService.shared.singIn(with: loginRequest) { error in
            if let error = error {
                AlertManager.showSignInErrorAlert(
                    on: self,
                    with: error)
                return
            }
